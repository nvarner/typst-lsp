...
use typst::doc::Frame;
use typst::eval::{eco_format, CastInfo, Tracer, Value};
use typst::geom::{round_2, Length, Numeric};
use typst::ide::{analyze_labels, Tooltip};
use typst::syntax::{ast, LinkedNode, Source, SyntaxKind};
use typst::util::pretty_comma_list;
use typst::World;
use typst_library::prelude::EcoString;

use super::analyze::analyze_expr;
use super::{plain_docs_sentence, summarize_font_family};

/// From `typst::ide::tooltip`, but passing `source` through and replacing calls to `World::main`
/// with `source`
pub fn tooltip(
    ...
    named_param_tooltip(world, &leaf)
        ...
        .or_else(|| expr_tooltip(world, source, &leaf))
}
...
fn expr_tooltip(
    ...
    let values = analyze_expr(world, source, ancestor);
...
fn named_param_tooltip(world: &(dyn World + 'static), leaf: &LinkedNode) -> Option<Tooltip> {
